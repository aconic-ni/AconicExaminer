...
export type UserRole = 'gestor' | 'aforador' | 'ejecutivo' | 'coordinadora' | 'admin' | 'agente' | 'digitador';
...
export interface AforoCase {
  ...
  motivoRectificacion?: string | null; // Detailed reason for rectification
...
export interface AforoCaseUpdate {
    ...
    field: keyof AforoCase | 'status_change' | 'creation' | 'incident_report';
    oldValue: any;
    newValue: any;
...
export interface WorksheetDocument {
  id: string; // Unique ID for the document entry in the sub-table
  type: string;
  number: string;
  isCopy: boolean;
}
