...
import { Loader2, Users, FileText, Package, Calendar, Sigma, DivideCircle } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const allowedRoles = ['aforador', 'coordinadora', 'admin'];

  const fetchData = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const q = query(collection(db, "examenesPrevios"), orderBy("ne", "desc"));
      ...
      setError("No se pudieron cargar los datos para el dashboard. Verifique los permisos de Firestore.");
    } finally {
      setIsLoading(false);
    }
  };
 ...
 const filterExams = useCallback((start: Date, end: Date) => {
  ...
  const dashboardData = useMemo(() => {
    if (isLoading) {
       return {
        ...
        productsByManager: [],
        examsByMonth: [],
        avgProductsPerExam: 0,
        avgExamsPerManager: 0
      };
    }
    
    // Initialize months for the line chart
    ...
    // Populate with actual data, using the broader filterExams logic
  ...
  }, [filteredExams, isLoading, filterType, selectedYear, allExams]);
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-background"><Loader2 className="h-12 w-12 animate-spin text-primary" /><p className="ml-4">Cargando datos del dashboard...</p></div>;
  }
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <Card>
            <CardHeader>
                <CardTitle className="text-2xl">Dashboard de Operaciones</CardTitle>
