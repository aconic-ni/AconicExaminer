...
import { Loader2, FilePlus, Search, Edit, Eye, History, PlusSquare, UserCheck, Inbox, AlertTriangle, Download, ChevronsUpDown, Info, CheckCircle, CalendarRange, Calendar, CalendarDays, ShieldAlert, BookOpen, FileCheck2, MessageSquare, View, Banknote, Bell as BellIcon } from 'lucide-react';
...
import type { Worksheet, AforoCase, AforadorStatus, AforoCaseStatus, DigitacionStatus, WorksheetWithCase, AforoCaseUpdate, PreliquidationStatus, IncidentType, LastUpdateInfo, ExecutiveComment, InitialDataContext, AppUser, SolicitudRecord } from '@/types';
...
import { FacturacionModal } from '@/components/executive/FacturacionModal';
...
export default function ExecutivePage() {
  ...
  const [selectedCaseForFacturacion, setSelectedCaseForFacturacion] = useState<AforoCase | null>(null);
  ...
  const fetchCases = useCallback(async () => {
    ...
    try {
        ...
        if (user.role === 'admin' || user.role === 'coordinadora' || user.role === 'supervisor') {
            ...
            q = query(aforoCasesRef, where('createdBy', 'in', uidsToQuery), orderBy('createdAt', 'desc'));
        } else {
            q = query(aforoCasesRef, where('createdBy', '==', user.uid), orderBy('createdAt', 'desc'));
        }

        const [casesSnap, worksheetsSnap, ...paymentSnaps] = await Promise.all([
            getDocs(q),
        ...
        const paymentsMap = new Map<string, SolicitudRecord[]>();
        
        paymentSnaps.forEach(snap => {
            snap.forEach(doc => {
                const payment = { solicitudId: doc.id, ...doc.data() } as SolicitudRecord;
                if (payment.examNe) {
                    const existing = paymentsMap.get(payment.examNe) || [];
                    paymentsMap.set(payment.examNe, [...existing, payment]);
                }
            });
        });

        const casesData = casesSnap.docs.map(caseDoc => {
            const caseItem = { id: caseDoc.id, ...caseDoc.data() } as AforoCase;
            const worksheet = caseItem.worksheetId ? worksheetsMap.get(caseItem.worksheetId) || null : null;
            const pagos = paymentsMap.get(caseItem.ne) || [];
            return { ...caseItem, worksheet, pagos };
        });
  ...
  const handleAssignAforador = async (caseId: string, aforadorName: string) => {
    ...
    try {
        ...
        fetchCases(); // Refetch data after assignment
    } catch (error) {
  ...
  const handleAutoSave = useCallback(async (caseId: string, field: keyof AforoCase, value: any) => {
    ...
    // Use JSON.stringify to compare objects and arrays, especially for dates.
    ...
    try {
        ...
        // Logic to store last update info for statuses
        ...
        fetchCases(); // Refetch data after saving
    } catch (error) {
...
}, [user, allCases, toast, fetchCases]);
 ...
 const handleSearch = () => {
  ...
  const handleExport = async () => {
    ...
    try {
        ...
        for (const caseItem of filteredCases) { // Use filteredCases to export only visible data
            ...
            // Fetch audit logs for every case in the filtered list
  ...
  return (
                                ...
                                <TableRow key={c.id} className={savingState[c.id] ? "bg-amber-100" : ""}>
                                      ...
                                      <div className="flex items-center gap-0.5">
                                        <DropdownMenu>
                                          <DropdownMenuTrigger asChild>
                                            <Button variant="ghost" className="h-8 w-8 p-0"><span className="sr-only">Abrir</span><PlusSquare className="h-4 w-4" /></Button>
                                          </DropdownMenuTrigger>
                                    ...
                                    <TableCell>
                                        ...
                                        {c.facturado ? (
                                            <Tooltip>
                                                <TooltipTrigger>
                                                    <Badge variant="default" className="cursor-help bg-green-600">Facturado</Badge>
                                                </TooltipTrigger>
                                                <TooltipContent>
                                                    <p>Cuenta: {c.cuentaDeRegistro}</p>
                                                    <p>Fecha: {formatDate(c.facturadoAt)}</p>
                                                </TooltipContent>
                                            </Tooltip>
                                        ) : (
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <div className="inline-block">
                                                        <Button 
                                                            size="sm"
                                                            onClick={() => setSelectedCaseForFacturacion(c)}
                                                            disabled={!c.fechaDespacho}
                                                        >
                                                            <FileCheck2 className="mr-2 h-4 w-4"/> Facturar
                                                        </Button>
                                                    </div>
                                                </TooltipTrigger>
                                                {!c.fechaDespacho && (
                                                    <TooltipContent>
                                                        <p>Debe ingresar la fecha de despacho primero.</p>
                                                    </TooltipContent>
                                                )}
                                            </Tooltip>
                                        )}
    ...
    {selectedCaseForFacturacion && (<FacturacionModal isOpen={!!selectedCaseForFacturacion} onClose={() => setSelectedCaseForFacturacion(null)} caseData={selectedCaseForFacturacion} />)}
