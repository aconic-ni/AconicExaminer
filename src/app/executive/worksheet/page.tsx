...
import { X, Loader2, PlusCircle, Trash2, CheckSquare, Square, Receipt, Check, ChevronsUpDown, RotateCcw, ArrowLeft } from 'lucide-react';
...
function WorksheetForm() {
  ...
  // States for adding other documents
  const [docType, setDocType] = useState('');
  const [docNumber, setDocNumber] = useState('');
  const [isOriginal, setIsOriginal] = useState(false);

  // State for new permit inputs
  ...
  const onSubmit = async (data: WorksheetFormData) => {
    ...
    try {
        ...
        const aforoCaseData: Partial<AforoCase> = {
            ...
            declarationPattern: data.patternRegime, merchandise: data.description, createdBy: user.uid, createdAt: creationTimestamp,
        ...
        const permissionError = new FirestorePermissionError({
            path: `batch write to worksheets/${neTrimmed} and potentially AforoCases/${neTrimmed}`, operation: 'create',
            ...
            <div className="pt-4 border-t">
                <h3 className="text-lg font-medium mb-2">Permisos Requeridos</h3>
                 <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_auto] gap-3 items-end mb-4 p-3 border rounded-md">
                    ...
                    <Button type="button" onClick={addPermit}><PlusCircle className="mr-2 h-4 w-4"/>Añadir Permiso</Button>
                 </div>
