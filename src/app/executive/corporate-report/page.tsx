...
export const corporateReportSchema = z.object({
  ...
  requiredPermits: z.array(z.any()).optional(), // Simplified for now
...
function CorporateReportForm() {
  ...
  const form = useForm<CorporateReportFormData>({
    ...
    defaultValues: {
      worksheetType: 'corporate_report',
      ne: '',
      reference: '',
      entryCustoms: '',
      dispatchCustoms: '',
  ...
  const onSubmit = async (data: CorporateReportFormData) => {
    ...
    try {
      ...
      const aforoCaseData: Partial<AforoCase> = {
        ...
        declarationPattern: 'corporate_report',
        merchandise: data.observations,
        createdBy: user.uid,
        createdAt: creationTimestamp,
        aforadorStatus: 'En revisión', // Default status
  ...
  return (
    <>
      <Card className="w-full max-w-4xl mx-auto custom-shadow">
        <CardHeader>
          <CardTitle className="text-2xl">Nuevo Reporte Consignatario Empresarial</CardTitle>
          <CardDescription>Complete la información para generar el nuevo reporte.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-4">
              ...
              {/* Table to display added facturas can be added here if needed */}
