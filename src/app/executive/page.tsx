...
export default function ExecutivePage() {
  ...
  const handleArchiveCase = async () => {
    ...
    try {
      ...
      // The onSnapshot listener will automatically remove the case from the list
    } catch (error) {
 ...
 const handleSearch = () => {
  ...
  return (
                                        ...
                                        <DropdownMenu>
                                          ...
                                          <DropdownMenuContent>
                                            ...
                                            {user?.role === 'admin' && (
                                              <>
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem onSelect={() => setCaseToArchive(c)} className="text-destructive focus:bg-destructive/10 focus:text-destructive">
                                                  <Archive className="mr-2 h-4 w-4" /> Archivar Caso
                                                </DropdownMenuItem>
                                              </>
                                            )}
                                          </DropdownMenuContent>
    ...
    {caseToAssignAforador && (
     ...
     <AlertDialog open={!!caseToArchive} onOpenChange={(isOpen) => !isOpen && setCaseToArchive(null)}>
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle>¿Está seguro de archivar este caso?</AlertDialogTitle>
                <AlertDialogDescription>
                Esta acción marcará el caso como archivado y lo ocultará de la vista principal. Podrá encontrarlo en la sección de control de registros.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel onClick={() => setCaseToArchive(null)}>Cancelar</AlertDialogCancel>
                <AlertDialogAction onClick={handleArchiveCase}>Sí, archivar</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
    </AlertDialog>
    </>
