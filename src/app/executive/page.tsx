...
import type { Worksheet, AforoCase, AforadorStatus, AforoCaseStatus, DigitacionStatus, WorksheetWithCase, AforoCaseUpdate, PreliquidationStatus, IncidentType, LastUpdateInfo, ExecutiveComment, InitialDataContext, AppUser } from '@/types';
...
export default function ExecutivePage() {
  ...
  const fetchCases = useCallback(() => {
    ...
    // Ejecutivo not in a group sees only their own cases.
    else {
        ...
        for (const caseDoc of querySnapshot.docs) {
            ...
            if (caseItem.worksheetId) {
                ...
                if (worksheetSnap.exists()) {
                    worksheetData = { id: worksheetSnap.id, ...worksheetSnap.data() } as Worksheet;
                }
            }
            casesData.push({ ...caseItem, worksheet: worksheetData });
        }
                                ...
                                <TableRow key={c.id} className={savingState[c.id] ? "bg-amber-100" : ""}>
                                    ...
                                    <TableCell>
                                      {c.resaNumber ? (
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                            <Badge variant="secondary" className="cursor-help">{c.resaNumber}</Badge>
                                          </TooltipTrigger>
                                          <TooltipContent>
                                            <p>Notificado: {formatDate(c.resaNotificationDate, false)}</p>
                                            <p>Vence: {formatDate(c.resaDueDate, false)}</p>
                                          </TooltipContent>
                                        </Tooltip>
                                      ) : 'N/A'}
                                    </TableCell>
                                    ...
                                    <TableCell>{c.facturaNumber || ''}</TableCell>
                                    <TableCell>
