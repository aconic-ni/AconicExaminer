...
const LastUpdateTooltip = ({ lastUpdate, caseCreation, assignedUser }: { lastUpdate?: LastUpdateInfo | null, caseCreation: Timestamp, assignedUser?: string | null }) => {
    if (!lastUpdate && !assignedUser) return null;
    ...
    return (
        <Tooltip>
            ...
            <TooltipContent>
                {assignedUser && <p>Asignado a: {assignedUser}</p>}
                {lastUpdate?.by && <p>{label}: {lastUpdate.by}</p>}
                {lastUpdate?.at && <p>Fecha: {formatDate(lastUpdate.at)}</p>}
            </TooltipContent>
        </Tooltip>
    );
};
                                ...
                                <TableRow key={c.id} className={savingState[c.id] ? "bg-amber-100" : (isResaCritical ? "bg-red-200 hover:bg-red-300" : "")}>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center">
                                            ...
                                            <LastUpdateTooltip lastUpdate={c.digitacionStatusLastUpdate} caseCreation={c.createdAt} assignedUser={c.digitadorAsignado}/>
                                        </div>
                                    </TableCell>
                                     <TableCell>
                                        <div className="flex items-center gap-2">
