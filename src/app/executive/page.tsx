...
export default function ExecutivePage() {
 ...
 const handleSearch = () => {
  ...
  const filteredCases = useMemo(() => {
    let baseFiltered = allCases;
    
    if (activeTab === 'worksheets') {
      baseFiltered = baseFiltered.filter(c => c.worksheet?.worksheetType === 'hoja_de_trabajo' || c.worksheet?.worksheetType === undefined);
    } else if (activeTab === 'anexos') {
      baseFiltered = baseFiltered.filter(c => c.worksheet?.worksheetType === 'anexo_5' || c.worksheet?.worksheetType === 'anexo_7');
    } else if (activeTab === 'corporate') {
      baseFiltered = baseFiltered.filter(c => c.worksheet?.worksheetType === 'corporate_report');
    }

    baseFiltered = baseFiltered.filter(c => !c.isArchived);
    ...
    if (appliedFilters.noFacturado && !appliedFilters.facturado) {
        baseFiltered = baseFiltered.filter(c => !c.facturado);
    } else if (appliedFilters.facturado && !appliedFilters.noFacturado) {
        baseFiltered = baseFiltered.filter(c => c.facturado === true);
    }
    ...
    if (appliedFilters.dateFilterType === 'today') {
        ...
        baseFiltered = Array.from(todaysCases.values()).filter(c => !c.isArchived);
    ...
    let columnFiltered = baseFiltered;
    if (neFilter) columnFiltered = columnFiltered.filter(c => c.ne.toLowerCase().includes(neFilter.toLowerCase()));
    if (ejecutivoFilter) columnFiltered = columnFiltered.filter(c => c.executive.toLowerCase().includes(ejecutivoFilter.toLowerCase()));
    if (consignatarioFilter) columnFiltered = columnFiltered.filter(c => c.consignee.toLowerCase().includes(consignatarioFilter.toLowerCase()));
    if (facturaFilter) {
      const lowerCaseFilter = facturaFilter.toLowerCase();
      columnFiltered = columnFiltered.filter(c => {
    ...
    if (selectividadFilter) columnFiltered = columnFiltered.filter(c => (c.selectividad || 'N/A').toLowerCase().includes(selectividadFilter.toLowerCase()));
    if (incidentTypeFilter) columnFiltered = columnFiltered.filter(c => getIncidentTypeDisplay(c).toLowerCase().includes(incidentTypeFilter.toLowerCase()));

    const finalData = columnFiltered.length > 0 || (!neFilter && !ejecutivoFilter && !consignatarioFilter && !facturaFilter && !selectividadFilter && !incidentTypeFilter)
        ? columnFiltered 
        : baseFiltered;

    return finalData.sort((a, b) => (b.createdAt?.toMillis() ?? 0) - (a.createdAt?.toMillis() ?? 0));
  ...
  const handleDocsPermisosClick = (caseId: string) => {
    router.push(`/managerpermisos?id=${caseId}`);
  };
                          ...
                          <div className="flex items-center gap-0.5">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                ...
                                <DropdownMenuItem onSelect={() => handleDocsPermisosClick(c.id)} disabled={!c}>
  ...
  return (
    ...
    {selectedCaseForDocs && (<ManageDocumentsForm caseData={selectedCaseForDocs} onClose={() => setSelectedCaseForDocs(null)} />)}
