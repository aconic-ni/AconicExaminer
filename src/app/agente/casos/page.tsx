...
import { Anexo5Details } from '@/components/executive/anexos/Anexo5Details';
...
export default function AgenteCasosPage() {
  ...
  const handleCloseObservationModal = (shouldRefetch: boolean) => {
  ...
  if (worksheetToView) {
      const isAnexo = worksheetToView.worksheetType === 'anexo_5' || worksheetToView.worksheetType === 'anexo_7';
      if (isAnexo) {
        return <AppShell><div className="py-2 md:py-5"><Anexo5Details worksheet={worksheetToView} onClose={() => setWorksheetToView(null)} /></div></AppShell>;
      }
      return <AppShell><div className="py-2 md:py-5"><WorksheetDetails worksheet={worksheetToView} onClose={() => setWorksheetToView(null)} /></div></AppShell>;
  }
                      ...
                      <TableRow key={caseItem.id}>
                        <TableCell>
                            <div className="flex items-center gap-1">
                                ...
                                <Button size="sm" variant="destructive" onClick={() => openActionModal(caseItem, 'observation')} disabled={caseItem.revisorStatus !== 'Pendiente'}>Rechazar</Button>
                                <Button size="sm" onClick={() => openActionModal(caseItem, 'observation')} disabled={caseItem.revisorStatus !== 'Pendiente'}>Aprobar</Button>
                            </div>
                        </TableCell>
      ...
      {selectedCaseForAction && (
          <>
            <ObservationModal 
                ...
                onClose={() => handleCloseObservationModal(true)}
