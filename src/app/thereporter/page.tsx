...
export default function TheReporterPage() {
  ...
  const canSendToDigitization = user?.role === 'aforador' || user?.role === 'admin' || user?.roleTitle === 'supervisor';
  ...
  const handleExport = async () => {
    ...
    // Unlike previous implementation, we now use the data already in memory (filteredCases)
    // and pass it to the exporter. This avoids N+1 reads.
    try {
        const auditLogs: (AforoCaseUpdate & { caseNe: string })[] = [];
        
        // We still need to fetch audit logs, this is an area for further optimization if needed
        // but it's less critical than fetching worksheet data for every row.
                        ...
                        <div className="border-t pt-4 space-y-4">
                            ...
                            <div className="flex justify-end gap-2">
                                ...
                                <Button variant={showPendingOnly ? 'secondary' : 'outline'} onClick={() => setShowPendingOnly(!showPendingOnly)}>
                                    <Filter className="mr-2 h-4 w-4" /> Ver Pendientes
                                </Button>
