...
import type { Worksheet, RequiredPermit } from '@/types';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { format } from 'date-fns';
import { es } from 'date-fns/locale';

interface PermitRow extends RequiredPermit {
  ne: string;
  reference?: string;
  executive: string;
}
...
export default function PermisosPage() {
  ...
  const [focusMode, setFocusMode] = useState(true); // Default to showing only pending/in-progress
  ...
  const filteredPermits = useMemo(() => {
    ...
    // Sort by status priority: Pendiente > En Trámite > Rechazado > Entregado
             ...
             <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div>
                 ...
                 <div className="relative w-full sm:max-w-md">
                    ...
                    <Input 
                        placeholder="Buscar por NE, Referencia, Factura, Ejecutivo o Permiso..."
                        className="pl-10"
                        value={searchTerm}
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-10 px-6 bg-secondary/30 rounded-lg">
                <p className="mt-1 text-muted-foreground">No se encontraron permisos con los filtros actuales.</p>
              </div>
            ) : (
