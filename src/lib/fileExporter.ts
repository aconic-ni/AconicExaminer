...
import type { ExamData, Product, ExamDocument, ExportableExamData, AforoCase, AforoCaseUpdate } from '@/types';
...
export function downloadAforoCasesAsExcel(cases: AforoCase[], auditLogs: (AforoCaseUpdate & { caseNe: string })[]) {
    const now = new Date();
    const fechaHoraExportacion = now.toLocaleString('es-NI', { dateStyle: 'long', timeStyle: 'short' });
    ...
    const caseHeaders = [
        "NE", "Consignatario", "Mercancía", "Patrón Dec.", "Aforador", "Fecha Asignación",
...
export function downloadExecutiveReportAsExcel(cases: AforoCase[], auditLogs: (AforoCaseUpdate & { caseNe: string })[]) {
  const now = new Date();
  const fechaHoraExportacion = now.toLocaleString('es-NI', { dateStyle: 'long', timeStyle: 'short' });
  ...
  const formatTimestamp = (ts: any, includeTime: boolean = true): string => {
      ...
      if (date instanceof Date && !isNaN(date.getTime())) {
          return includeTime 
              ...
              : date.toLocaleDateString('es-NI', { dateStyle: 'short'});
      }
      return 'Fecha Inválida';
  };
  ...
  const headers = [
      "NE", "INICIO DE OPERACION", "CONSIGNATARIO", "FECHA Y HORA DE ASIGNACION DE AFORO",
      "ESTATUS DE REVISOR", "ESTATUS DIGITACION", "SELECTIVO", "FECHA DE DESPACHO",
      "INCIDENCIAS", "FACTURADO", "FECHA Y HORA DE FACTURADO"
  ];
