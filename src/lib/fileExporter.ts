...
export function downloadAforoCasesAsExcel(cases: (AforoCase & { dispatchCustoms?: string })[], auditLogs: (AforoCaseUpdate & { caseNe: string })[]) {
    const now = new Date();
    const fechaHoraExportacion = format(now, 'dd/MM/yy HH:mm', { locale: es });
    ...
    // --- Hoja 1: Casos de Aforo ---
    const caseHeaders = [
        "NE", "Consignatario", "Mercancía", "Modelo (Patrón)", "Aforador", "Fecha Asignación",
        "Total Posiciones", "Entregado a Aforo", "Revisor Asignado", "Estado Revisor",
        "Observación Revisor", "Estado Aforador", "Comentario Aforador", "Estado Digitación",
        "Digitador Asignado", "Fecha Asign. Digitador", "Comentario Digitación", "Declaración Aduanera",
        "Incidencia Reportada", "Motivo Incidencia", "Estado Incidencia"
    ];
    ...
    const caseRows = cases.map(c => [
        ...
        c.revisorStatus || 'Pendiente', c.observacionRevisor || '', c.aforadorStatus || 'Pendiente',
        c.aforadorComment || '', c.digitacionStatus || 'N/A', c.digitadorAsignado || 'N/A',
        formatTimestamp(c.digitadorAsignadoAt), c.digitacionComment || '', c.declaracionAduanera || 'N/A',
        c.incidentReported ? 'Sí' : 'No', c.incidentReason || '', c.incidentStatus || 'N/A'
    ]);

    const ws_cases_data = [
        [`Reporte de Casos de Aforo - Customs Reports`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        caseHeaders,
        ...caseRows
    ];
    const ws_cases = XLSX.utils.aoa_to_sheet(ws_cases_data);
    const caseColWidths = caseHeaders.map((header, i) => ({
        wch: Math.max(header.length, ...caseRows.map(row => String(row[i] || '').length)) + 2
    }));
    ws_cases['!cols'] = caseColWidths;
    ...
    const logRows = auditLogs.map(log => [
        ...
        log.updatedBy,
        log.field,
        formatValueForExcel(log.oldValue),
        formatValueForExcel(log.newValue),
        log.comment || ''
    ]);

    const ws_logs_data = [
        [`Bitácora de Cambios - Customs Reports`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        logHeaders,
        ...logRows
    ];
    const ws_logs = XLSX.utils.aoa_to_sheet(ws_logs_data);
    const logColWidths = logHeaders.map((header, i) => ({
        wch: Math.max(header.length, ...logRows.map(row => String(row[i] || '').length)) + 2
    }));
    ws_logs['!cols'] = logColWidths;
    
    // --- Hoja 3: Casos con Complementaria (Rectificación) ---
    const rectificationCases = cases.filter(c => c.incidentType === 'Rectificacion');
    const rectificationHeaders = [
        "NE", "CONSIGNATARIO", "DECLARACION ADUANERA", "PAGO INICIAL (SI/NO)", 
        "RECIBO DE CAJA PAGO INICIAL", "MONTO DE DECLARACION INICIAL", "ADUANA DE DESPACHO",
        "MOTIVO DE LA RECTIFICACION", "OBSERVACIONES"
    ];
    const rectificationRows = rectificationCases.map(c => [
        c.ne,
        c.consignee,
        c.declaracionAduanera || 'N/A',
        c.pagoInicialRealizado ? 'Sí' : 'No',
        c.reciboDeCajaPagoInicial || 'N/A',
        c.montoPagoInicial && typeof c.montoPagoInicial === 'number' ? `$${c.montoPagoInicial.toFixed(2)}` : 'N/A',
        (c as any).dispatchCustoms || 'N/A',
        c.motivoRectificacion || '',
        c.observaciones || ''
    ]);
    
    const ws_rect_data = [
        [`Reporte de Casos con Declaración Complementaria (Rectificación)`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        rectificationHeaders,
        ...rectificationRows
    ];
    const ws_rect = XLSX.utils.aoa_to_sheet(ws_rect_data);
    const rectColWidths = rectificationHeaders.map((header, i) => ({
      wch: Math.max(header.length, ...rectificationRows.map(row => String(row[i] || '').length)) + 2
    }));
    ws_rect['!cols'] = rectColWidths;

    // --- Crear y descargar libro ---
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws_cases, "Casos de Aforo");
    XLSX.utils.book_append_sheet(wb, ws_rect, "Casos con Complementaria");
    XLSX.utils.book_append_sheet(wb, ws_logs, "Bitácora de Cambios");
    XLSX.writeFile(wb, `Reporte_Casos_Aforo_${now.toISOString().split('T')[0]}.xlsx`);
}

export async function downloadExecutiveReportAsExcel(cases: AforoCase[], auditLogs: (AforoCaseUpdate & { caseNe: string })[]) {
    const now = new Date();
    const fechaHoraExportacion = format(now, 'dd/MM/yy HH:mm', { locale: es });

    const formatTimestamp = (ts: any, includeTime: boolean = true): string => {
        if (!ts) return 'N/A';
        const date = ts.toDate ? ts.toDate() : new Date(ts);
        if (date instanceof Date && !isNaN(date.getTime())) {
    ...
    const mainReportHeaders = [
      "NE", "INICIO DE OPERACION", "CONSIGNATARIO", "FACTURA", "ASIGNACION DE AFORO",
      "ESTATUS DE REVISOR", "ESTATUS DIGITACION", "TIPO DOC. TRANSPORTE", "COMPAÑIA TRANSPORTE", 
      "NUMERO DOC. TRANSPORTE", "NUMERO RESA",
      "SELECTIVO", "FECHA DE DESPACHO",
      "TIPO INCIDENCIA", "FACTURADO", "FECHA DE FACTURACION", "CUENTA DE REGISTRO",
      "ESTATUS EJECUTIVO (AMPLIADO)"
    ];

    const mainReportRowsPromises = cases.map(async c => {
        const executiveCommentsText = c.executiveComments
            ? c.executiveComments
                .slice()
                .reverse()
                .map(comment => `(${comment.author}): ${comment.text} (${format(comment.createdAt.toDate(), 'dd/MM/yy hh:mm a')})`)
                .join('\n')
            : '';

        let worksheetData: Worksheet | null = null;
        if (c.worksheetId) {
            const wsDocRef = doc(db, 'worksheets', c.worksheetId);
            const wsSnap = await getDoc(wsDocRef);
            if (wsSnap.exists()) {
                worksheetData = wsSnap.data() as Worksheet;
            }
        }
        ...
        return [
            ...
            worksheetData?.transportDocumentType || 'N/A',
            worksheetData?.transportCompany || 'N/A',
            worksheetData?.transportDocumentNumber || 'N/A',
            worksheetData?.resa || 'N/A',
            c.selectividad || '',
            ...
            c.cuentaDeRegistro || '',
            executiveCommentsText
        ];
    });

    const mainReportRows = await Promise.all(mainReportRowsPromises);
    ...
    const ws_main = XLSX.utils.aoa_to_sheet(ws_main_data);
    const mainColWidths = mainReportHeaders.map((header, i) => ({
        wch: Math.max(header.length, ...mainReportRows.map(row => String(row[i] || '').length)) + 2
    }));
    ws_main['!cols'] = mainColWidths;

    // --- Hoja 2: Duda de Valor ---
    const valueDoubtCases = cases.filter(c => c.hasValueDoubt);
    const valueDoubtHeaders = [
        "NE", "Consignatario", "Declaración", "Aduana Despacho", "Monto de Preliquidación (USD)",
        "Proceso", "Levante Solicitado", "Ampliación de Plazo", "Vencimiento de Caso", "Asignado a Legal"
    ];
    const valueDoubtRows = valueDoubtCases.map(c => [
        c.ne,
        c.consignee,
        c.declaracionAduanera || 'N/A',
        (c as any).dispatchCustoms || 'N/A',
        c.valueDoubtAmount ? `$${c.valueDoubtAmount.toFixed(2)}` : 'N/A',
        c.valueDoubtStatus || 'N/A',
        c.valueDoubtLevanteRequested ? 'Sí' : 'No',
        c.valueDoubtExtensionRequested ? 'Sí' : 'No',
        formatTimestamp(c.valueDoubtDueDate, false),
        c.valueDoubtAssignedToLegal ? 'Sí' : 'No',
    ]);

    const ws_vd_data = [
        [`Reporte de Casos con Duda de Valor`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        valueDoubtHeaders,
        ...valueDoubtRows
    ];
    const ws_vd = XLSX.utils.aoa_to_sheet(ws_vd_data);
    const vdColWidths = valueDoubtHeaders.map((header, i) => ({
        wch: Math.max(header.length, ...valueDoubtRows.map(row => String(row[i] || '').length)) + 2
    }));
    ws_vd['!cols'] = vdColWidths;
    ...
    const rectificationHeaders = [
        ...
        "RECIBO DE CAJA PAGO INICIAL", "MONTO DE DECLARACION INICIAL", "ADUANA DE DESPACHO",
        "MOTIVO DE LA RECTIFICACION", "OBSERVACIONES",
        "ESTADO INCIDENCIA", "REVISADO POR", "FECHA REVISIÓN"
    ];
    const rectificationRows = rectificationCases.map(c => [
        c.ne,
        c.consignee,
        ...
        c.reciboDeCajaPagoInicial || 'N/A',
        c.montoPagoInicial && typeof c.montoPagoInicial === 'number' ? `$${c.montoPagoInicial.toFixed(2)}` : 'N/A',
        c.dispatchCustoms || 'N/A',
        c.motivoRectificacion || '',
        c.observaciones || '',
        c.incidentStatus || 'Pendiente',
        c.incidentReviewedBy || 'N/A',
        formatTimestamp(c.incidentReviewedAt),
    ]);
    ...
    const ws_logs_data = [
        [`Bitácora de Cambios - Reporte Ejecutivo`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        logHeaders,
        ...logRows
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws_main, "Reporte Ejecutivo");
    XLSX.utils.book_append_sheet(wb, ws_vd, "Casos con Duda de Valor");
    XLSX.utils.book_append_sheet(wb, ws_rect, "Casos con Complementaria");
    XLSX.utils.book_append_sheet(wb, ws_logs, "Bitácora de Cambios");
