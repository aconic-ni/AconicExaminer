...
export async function downloadExecutiveReportAsExcel(cases: (AforoCase & { dispatchCustoms?: string })[], auditLogs: (AforoCaseUpdate & { caseNe: string })[]) {
    ...
    const caseHeaders = [
        "NE", "Consignatario", "Mercancía", "Modelo (Patrón)", "Aduana Despacho", "Aforador", "Fecha Asignación",
        ...
        "Incidencia Reportada", "Motivo Incidencia", "Estado Incidencia", "Facturado", "Cuenta de Registro"
    ];

    const caseRows = cases.map(c => [
        c.ne, c.consignee, c.merchandise, c.declarationPattern, c.dispatchCustoms || 'N/A', c.aforador, formatTimestamp(c.assignmentDate),
        ...
        c.facturado ? 'Sí' : 'No', c.cuentaDeRegistro || 'N/A'
    ]);

    const ws_cases_data = [
        [`Reporte de Casos Ejecutivos - Customs Reports`],
        [`Generado el: ${fechaHoraExportacion}`],
        [],
        caseHeaders,
        ...caseRows
