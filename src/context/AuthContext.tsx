...
export const AuthProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const checkUserProfile = useCallback(async (firebaseUser: FirebaseUser) => {
    ...
    if (docSnap.exists()) {
      ...
      let visibilityGroupDisplayNames: string[] = [];
      if (userData.visibilityGroup && userData.visibilityGroup.length > 0) {
          try {
            const usersQuery = query(collection(db, "users"), where('__name__', 'in', userData.visibilityGroup));
            const userDocs = await getDocs(usersQuery);
            visibilityGroupDisplayNames = userDocs.docs.map(d => d.data().displayName).filter(Boolean);
          } catch (e) {
            console.error("Error fetching visibility group display names:", e);
          }
      }
      ...
      setUser({
        ...
        visibilityGroup: visibilityGroupDisplayNames,
