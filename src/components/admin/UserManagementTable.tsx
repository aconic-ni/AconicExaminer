...
const userRoles: UserRole[] = ['admin', 'coordinadora', 'ejecutivo', 'gestor', 'aforador', 'agente', 'supervisor', 'digitador', 'revisor', 'calificador', 'autorevisor', 'autorevisor_plus'];
        ...
        <Table>
            <TableHeader>
                <TableRow>
                    ...
                    <TableHead>Título de Rol</TableHead>
                    <TableHead>Acceso Reportes</TableHead>
                    <TableHead>Acceso Pagos</TableHead>
                    <TableHead>Grupo Ejecutivo</TableHead>
                </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={user.uid} className={savingState[user.uid] ? "bg-amber-100" : ""}>
                    ...
                    <TableCell>{user.displayName || <span className="text-muted-foreground italic">Sin nombre</span>}</TableCell>
                    <TableCell>
                        <Select
                            value={user.role || ''}
                    ...
                    <TableCell>
                        <Input
                            ...
                            onBlur={(e) => handleUpdate(user.uid, 'roleTitle', e.target.value)}
                            disabled={savingState[user.uid]}
                            placeholder="Ej: Agente Aduanero"
                        />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                        {user.role === 'ejecutivo' && (
                            <Button variant="outline" size="sm" onClick={() => setSelectedUserForGroup(user)}>
                                <Users className="mr-2 h-4 w-4" /> Gestionar Grupo
                            </Button>
                        )}
                    </TableCell>
                </TableRow>
