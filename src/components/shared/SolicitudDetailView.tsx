...
export default function SolicitudDetailView({ id, isInlineView, onBackToList, children, solicitud: initialSolicitud }: SolicitudDetailViewProps) {
  ...
  const [initialData, setInitialData] = useState<InitialDataContext | null>(null);
  const [isLoading, setIsLoading] = useState(!initialSolicitud);
  const [error, setError] = useState<string | null>(null);
  
  useEffect(() => {
    if (initialSolicitud) { // Data is passed directly (e.g., from database page)
      ...
      setIsLoading(false);
    } else if (id && contextSolicitudes) { // From live context (examinerPay)
        const foundSolicitud = contextSolicitudes.find(s => s.id === id);
    ...
    } else { // Fallback for edge cases or if no ID is passed
        setError("No hay datos de solicitud para mostrar.");
        setIsLoading(false);
    }
  }, [id, initialSolicitud, contextSolicitudes, contextInitialData]);
    ...
    return (
        <CardContent className="pt-4 print:p-0">
             ...
             <div className="no-print">
              ...
              <div className="mb-3 p-4 border rounded-md bg-secondary/30 print:border print:mb-2 print:grid-cols-2 print:gap-x-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-0 print:grid-cols-2 print:gap-x-8">
                    ...
                    <DetailItem label="Referencia" value={initialData.reference || 'N/A'} icon={FileText} className="md:col-span-2 print:col-span-2"/>
                  </div>
              </div>
              ...
              <div className="hidden print:block">
                <Image src={`${basePath}/imagenes/FOOTERSOLICITUDETAIL.svg`} alt="Footer Solicitud Detail" width={800} height={100} className="w-full h-auto object-contain mt-6" data-ai-hint="company seal official" />
              </div>
  ...
  const mainCard = (
    <Card id="printable-area" className="w-full max-w-4xl mx-auto shadow-lg card-print-styles">
