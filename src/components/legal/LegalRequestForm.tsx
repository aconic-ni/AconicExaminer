...
export function LegalRequestForm() {
  ...
  useEffect(() => {
    ...
    if (ne) form.setValue('ne', ne);
    if (consignee) form.setValue('consignee', consignee);
    if (serviceType) {
        ...
        if (serviceType === 'Dictamen Tecnico INE') {
            ...
            newService.equipoType = searchParams.get('tipoEquipo') as 'Refrigerador' | 'Aire Acondicionado' | undefined;
        }
        append(newService);
