...
export function DigitizationCasesTable({ filters, setAllFetchedCases, displayCases }: DigitizationCasesTableProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getDocs(qIncomplete), getDocs(qCompletedToday)])
        .then(([incompleteSnap, completedTodaySnap]) => {
            const incompleteCases = incompleteSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as AforoCase));
            const completedTodayCases = completedTodaySnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as AforoCase));
            ...
            uniqueCases.sort((a, b) => (b.createdAt?.toMillis() ?? 0) - (a.createdAt?.toMillis() ?? 0));
            
            setAllFetchedCases(uniqueCases);
            setIsLoading(false);
        })
        .catch(error => {
