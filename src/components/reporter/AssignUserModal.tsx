...
export function AssignUserModal({ isOpen, onClose, caseData, assignableUsers, onAssign, title, description }: AssignUserModalProps) {
  ...
  // Aforador role shouldn't be assigning from this modal in the /thereporter view.
  const canAssign = currentUser?.role === 'admin' || currentUser?.role === 'coordinadora' || currentUser?.roleTitle === 'supervisor' || currentUser?.role === 'ejecutivo';
