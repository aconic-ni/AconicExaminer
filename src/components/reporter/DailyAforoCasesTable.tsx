...
export function DailyAforoCasesTable({ filters, setFilteredCases }: DailyAforoCasesTableProps) {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      // Base query
      const casesQuery = query(collection(db, "AforoCases"), orderBy("createdAt", "desc"));
  ...
  }, [filters, setFilteredCases, toast]);
  ...
  const canEditFields = user?.role === 'admin' || user?.role === 'coordinadora';
      ...
      <Table>
        ...
        <TableBody>
          {cases.map((caseItem) => {
            ...
            const canEditThisAforadorStatus = canEditFields || (user?.role === 'aforador' && user?.displayName === caseItem.aforador);
               ...
               <TableCell className="text-center">
                 <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        { (canEditThisAforadorStatus || canEditFields) && (
                        ...
                        { (user?.role === 'aforador' || user?.role === 'admin') && caseItem.revisorStatus === 'Rechazado' && (
                         ...
                         { (user?.role === 'admin' || user?.role === 'coordinadora') && caseItem.revisorStatus === 'Aprobado' && (
              ...
              <TableCell>
                 <div className="flex items-center gap-1">
                    <Select
                        ...
                        disabled={!canEditThisAforadorStatus}
                    >
                ...
                <TableRow className="bg-muted/30 hover:bg-muted/40">
                  <TableCell colSpan={9} className="p-0">
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                       ...
                       <div>
                        <label className="text-xs font-medium text-muted-foreground">Asignar Aforador</label>
                         <Select
                            ...
                            onValueChange={(value) => handleAutoSave(caseItem.id, 'aforador', value)}
                      ...
                      <div>
                        <label className="text-xs font-medium text-muted-foreground">Fecha de Asign/Aforo</label>
                        <DatePickerWithTime
                          date={(caseItem.assignmentDate as Timestamp)?.toDate()}
                          onDateChange={(date) => handleAutoSave(caseItem.id, 'assignmentDate', date)}
                        />
                      </div>
                       <div>
                        <label className="text-xs font-medium text-muted-foreground">Entregado a Aforo</label>
                         <DatePickerWithTime
                            ...
                            onDateChange={(date) => handleAutoSave(caseItem.id, 'entregadoAforoAt', date ? Timestamp.fromDate(date) : null)}
                         />
                      </div>
                    </div>
                  </TableCell>
