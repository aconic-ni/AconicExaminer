...
import { collection, query, where, onSnapshot, Timestamp, orderBy, doc, updateDoc, addDoc, getDocs, getDoc, collectionGroup, writeBatch } from 'firebase/firestore';
import { useAuth } from '@/context/AuthContext';
import type { AforoCase, AforoCaseStatus, AforoCaseUpdate, AppUser, AforadorStatus, Worksheet, DigitacionStatus, IncidentStatus, PreliquidationStatus, LastUpdateInfo } from '@/types';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Loader2, Inbox, Eye, History, Repeat, PlusSquare, Edit, BookOpen, ChevronDown, ChevronRight, User, UserCheck, Send, AlertTriangle, CheckSquare, ChevronsUpDown, Check, Info, Users } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchAssignableUsers = async () => {
        ...
        const roleQueries = [
            ...
            query(collection(db, 'users'), where('role', '==', 'ejecutivo')),
    ...
    let q = query(collection(db, "AforoCases"));

    if (filters.ne) {
        q = query(q, where('ne', '==', filters.ne.toUpperCase()));
    }
    if (filters.consignee) {
        q = query(q, where('consignee', '==', filters.consignee));
    }
