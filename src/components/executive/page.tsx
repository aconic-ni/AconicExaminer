...
import { Loader2, FilePlus, Search, Edit, Eye, History, PlusSquare, UserCheck, Inbox, AlertTriangle, Download, ChevronsUpDown, Info, CheckCircle, CalendarRange, Calendar, CalendarDays, ShieldAlert, BookOpen, FileCheck2, MessageSquare, View, Banknote, Bell as BellIcon, RefreshCw } from 'lucide-react';
...
import type { Worksheet, AforoCase, AforadorStatus, AforoCaseStatus, DigitacionStatus, WorksheetWithCase, AforoCaseUpdate, PreliquidationStatus, IncidentType, LastUpdateInfo, ExecutiveComment, InitialDataContext, AppUser, SolicitudRecord, ExamDocument } from '@/types';
  ...
  useEffect(() => {
   ...
   const fetchCases = useCallback(async () => {
    ...
    if (user.role === 'ejecutivo' && user.visibilityGroup && user.visibilityGroup.length > 0) {
      ...
      q = query(collection(db, 'AforoCases'), where('executive', 'in', groupDisplayNames), orderBy('createdAt', 'desc'));
