...
export const WorksheetDetails: React.FC<{ worksheet: Worksheet; onClose: () => void; }> = ({ worksheet, onClose }) => {
  ...
  const getEditLink = () => {
    if (!worksheet.worksheetType || !worksheet.id) return '/executive';

    switch (worksheet.worksheetType) {
        case 'hoja_de_trabajo':
            return `/executive/worksheet?id=${worksheet.id}`;
        case 'anexo_5':
        case 'anexo_7':
            return `/executive/anexos?type=${worksheet.worksheetType}&id=${worksheet.id}`;
        case 'corporate_report':
            return `/executive/corporate-report?id=${worksheet.id}`;
        default:
            return '/executive';
    }
  };
  
  if (worksheet.worksheetType === 'anexo_5' || worksheet.worksheetType === 'anexo_7') {
