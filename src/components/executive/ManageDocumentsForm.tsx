...
import { Loader2, PlusCircle, Trash2, FileText, Calendar, Receipt, RotateCcw, MessageSquare, Info, Scale, Settings, ArrowLeft } from 'lucide-react';
                                 ...
                                 return (
                                     <TableRow key={permit.id}>
                                         ...
                                         <TableCell className="text-right">
                                             ...
                                             {permit.name === 'Dictamen Tecnico INE' && originalWorksheet && (
                                                <Button variant="ghost" size="icon" asChild>
                                                    <Link href={`/legal/request?ne=${originalWorksheet.ne}&consignee=${encodeURIComponent(originalWorksheet.consignee)}&serviceType=${encodeURIComponent(permit.name)}&factura=${encodeURIComponent(permit.facturaNumber || '')}&item=${encodeURIComponent(permit.item || '')}&marca=${encodeURIComponent(permit.marcaEquipo || '')}&modelo=${encodeURIComponent(permit.modeloEquipo || '')}&tipoEquipo=${encodeURIComponent(permit.equipoType || '')}`}>
