...
import { Loader2, PlusCircle, Trash2, FileText, Calendar, Receipt, RotateCcw, MessageSquare, Info } from 'lucide-react';
  ...
  const onSubmit = async (data: ManageDocsFormData) => {
    ...
    const originalPermits = originalWorksheet.requiredPermits || [];
     sanitizedData.requiredPermits.forEach(newPermit => {
        ...
        if (!oldPermit) { // This is a new permit
            const logRef = doc(updatesSubcollectionRef);
            batch.set(logRef, {
                updatedAt: Timestamp.now(), updatedBy: user.displayName, field: 'document_update',
                oldValue: null, newValue: `Permiso añadido: ${newPermit.name}`,
                comment: `Ejecutivo añadió un nuevo permiso requerido.`
            } as AforoCaseUpdate);
            return;
        }; 
                            ...
                            <TableCell>
                               <div className="flex items-center gap-2">
                                  ...
                                  <Select onValueChange={(value) => updatePermitField(index, {...currentPermit, status: value as DocumentStatus})} value={currentPermit.status}>
                                   ...
                                   {currentPermit.status === 'Entregado' && (
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Info className="h-4 w-4 text-muted-foreground cursor-help" />
                                            </TooltipTrigger>
                                            <TooltipContent>
                                                <p>Entregado, sig. que el permiso fue finalizado exitosamente y ya se encuentra en agencia/frontera el original</p>
                                            </TooltipContent>
                                        </Tooltip>
                                    )}
                                  {currentPermit.status === 'Rechazado' && (
