...
import { Loader2, PlusCircle, Trash2, FileText, Calendar, Receipt, RotateCcw, MessageSquare, Info, Scale, Settings } from 'lucide-react';
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-6 py-4">
            ...
            <div className="pt-4 border-t">
                ...
                <div className="space-y-4 p-3 border rounded-md">
                 ...
                 {originalWorksheet && originalWorksheet.requiredPermits && originalWorksheet.requiredPermits.length > 0 ? (
                    <div className="rounded-md border mt-4">
                        <Table>
                            ...
                            <TableBody>
                                {originalWorksheet.requiredPermits.map((permit, index) => {
                                 const isInePermit = permit.name === 'Dictamen Tecnico INE';
                                 return (
                                     <TableRow key={permit.id}>
                                         ...
                                         {groupMembers.length > 0 && (
                                             <TableCell>
                                                 <Controller
                                                     ...
                                                     <Select onValueChange={(value) => handlePermitFieldUpdate(index, 'assignedExecutive', value)} defaultValue={controllerField.value || user?.displayName || ''}>
                                         ...
                                         <TableCell className="text-right">
                                             ...
                                             {permit.name === 'Dictamen Tecnico INE' && (
                                                <Button variant="ghost" size="icon" asChild>
                                                    <Link href={`/legal/request?ne=${caseData.ne}&consignee=${encodeURIComponent(caseData.consignee)}&serviceType=${encodeURIComponent(permit.name)}&factura=${encodeURIComponent(permit.facturaNumber || '')}&item=${encodeURIComponent(permit.item || '')}&marca=${encodeURIComponent(permit.marcaEquipo || '')}&modelo=${encodeURIComponent(permit.modeloEquipo || '')}&tipoEquipo=${encodeURIComponent(permit.equipoType || '')}`}>
