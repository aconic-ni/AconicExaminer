...
import { InePermitDetailsModal } from '@/components/executive/worksheet/InePermitDetailsModal';
...
const worksheetSchema = z.object({
  worksheetType: z.enum(['hoja_de_trabajo', 'anexo_5', 'anexo_7']).default('hoja_de_trabajo'),
...
function WorksheetForm() {
  ...
  const [editingInePermit, setEditingInePermit] = useState<{ permit: RequiredPermit, index: number } | null>(null);
  ...
  const [ineItem, setIneItem] = useState('');
  const [ineMarca, setIneMarca] = useState('');
  const [ineModelo, setIneModelo] = useState('');
  const [ineEquipoType, setIneEquipoType] = useState<'Refrigerador' | 'Aire Acondicionado' | undefined>(undefined);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
        ...
        const aforadorQuery = query(collection(db, 'users'), where('role', '==', 'aforador'));
  ...
  const addPermit = () => {
    ...
    const newPermit: Partial<RequiredPermit> = { 
      id: uuidv4(), 
    ...
    if (finalPermitName === 'Dictamen Tecnico INE') {
      newPermit.item = ineItem;
      newPermit.marcaEquipo = ineMarca;
      newPermit.modeloEquipo = ineModelo;
      newPermit.equipoType = ineEquipoType;
    }

    if (permitStatus === 'Entregado') {
      appendDoc({ id: uuidv4(), type: finalPermitName, number: 'VER BITACORA', isCopy: false, status: 'Entregado' });
    } else {
      appendPermit(newPermit as RequiredPermit);
    }
  ...
  const handleSaveIneDetails = (index: number, updatedPermit: RequiredPermit) => {
    updatePermitField(index, updatedPermit);
    setEditingInePermit(null);
    toast({ title: "Detalles Guardados", description: "La información del dictamen INE ha sido actualizada." });
  }
  ...
  return (
                ...
                <h3 className="text-lg font-medium mb-2">Permisos Requeridos</h3>
                 <div className="space-y-4 p-3 border rounded-md">
                    <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_auto] gap-3 items-end">
                     ...
                     {permitName === 'Dictamen Tecnico INE' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-muted">
                            <div><Label>Item</Label><Input value={ineItem} onChange={e => setIneItem(e.target.value)} /></div>
                            <div><Label>Marca de Equipo</Label><Input value={ineMarca} onChange={e => setIneMarca(e.target.value)} /></div>
                            <div><Label>Modelo de Equipo</Label><Input value={ineModelo} onChange={e => setIneModelo(e.target.value)} /></div>
                            <div><Label>Tipo de Equipo</Label>
                                <Select value={ineEquipoType} onValueChange={(v) => setIneEquipoType(v as any)}>
                 ...
                 {permitFields.length > 0 && (
                    <div className="rounded-md border mt-4"><Table>
                        ...
                        <TableBody>{permitFields.map((field, index) => {
                            const currentPermit = form.getValues(`requiredPermits.${index}`);
                            const isInePermit = field.name === 'Dictamen Tecnico INE';
                            return (
                            <TableRow key={field.id}>
                                ...
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-1">
                                      {isInePermit && (
                                        <Button type="button" variant="ghost" size="icon" onClick={() => setEditingInePermit({ permit: field, index })}>
    ...
    {editingInePermit && (
        <InePermitDetailsModal
            isOpen={!!editingInePermit}
            onClose={() => setEditingInePermit(null)}
            permit={editingInePermit.permit}
            onSave={(updatedPermit) => handleSaveIneDetails(editingInePermit.index, updatedPermit)}
        />
    )}
    </>
  )
